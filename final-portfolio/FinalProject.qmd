...
```{r}
  #| eval: false
disaster_raw <- read_csv("../final-portfolio/data_raw/DISASTERS/1900_2021_DISASTERS.xlsx - emdat data.csv")
```
...
write_csv(Graph_ResearchQ1, file = "data_clean/ResearchQ1_clean.csv") 
```
...
```{r}
Graph_ResearchQ1 <- read_csv("../final-portfolio/data_clean/ResearchQ1_clean.csv")
```


```{r}
ggplot(Graph_ResearchQ1, aes(x = reorder(Region, damage_avg), 
                             y = death_avg, fill = Region)) +
...
```{r}
ResearchQ2_clean <- read_csv("../final-portfolio/data_clean/ResearchQ2_clean.csv")
```
